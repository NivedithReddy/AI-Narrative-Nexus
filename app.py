"""
advanced_text_analysis_app_fixed.py
Single-file Streamlit app — robust, production-ready, resource-aware.
Features:
- Multi-document input (paste / multiple .txt / CSV 'text' column)
- Preprocessing (cleaning, optional spaCy lemmatization, bigrams)
- TF-IDF keywords
- Safe NMF (guards against invalid n_components)
- Optional LDA (CountVectorizer)
- Optional KMeans clustering + SVD (lightweight)
- Optional 2D visualization (SVD -> t-SNE) (can be disabled)
- WordCloud
- VADER sentiment
- spaCy NER (if installed)
- Optional summarization (transformers) with graceful fallback
- Export results
- Light mode toggle to reduce CPU/RAM usage
"""
